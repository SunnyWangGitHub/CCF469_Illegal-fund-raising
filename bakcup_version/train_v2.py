...
def data_process():
    ...
    #     print('#############')
    #     print(base[j].value_counts())
    #     print('---------------')
    #     print('unique  :',len(base[j].value_counts().unique()))
    #     print('---------------')
    #     print(base[j].isnull().sum(axis=0))
    #     print('#############')
    ...
    # drop = ['opto','opform','WEBSITSIGN','FUNDAM','MEMNUM','FARNUM','ANNNEWMEMNUM','ANNREDMEMNUM']  # 0.83154018669 
    
    drop = [
            ###### base
            'oplocdistrict', # 行政区划代码 unique 14
            'industryco',  # 行业细类代码 unique 97
            'dom',     # 经营地址,
            'opscope', #经营范围 unique 39
            # 'enttype', # 企业类型 unique 15
            'enttypeitem', # 企业类型小类 unique 21 miss 8214
            # 'state', #状态 unique 6
            # 'orgid', # 机构标识 unique 65
            # 'jobid', # 职位标识 unique 127
            # 'adbusign', # 是否广告经营 unique 2
            # 'townsign', # 是否城镇 unique 2
            # 'regtype',# 主题登记类型 unique 3
            # 'empnum', # 从业人数 unique 31 miss 5250
            'compform',#组织形式 unique 2  miss 14234
            'parnum', #合伙人数, unique 2 miss 22526
            'exenum', #执行人数 unique 2  miss 23487
            'opform',# 经营期限止起 unique 8 miss 15865
            'opto',#经营期限止止
            'ptbusscope',#兼营范围' unique 0 miss 24865
            #'venind',#风险行业,unique 3 miss 16428
            'enttypeminu',#企业类型细类 unique 16 miss 17595
            'midpreindcode',#中西部优势产业代码 miss all
            'protype',#项目类型 unique 2 miss 24831
            'oploc',#经营场所 unique 15 miss 0
            #'regcap',#注册资本 unique 58 miss 191
            'reccap',#实缴资本 unique 35 miss 17781
            'forreccap',#实缴资本（外方） miss 24615
            'congro',# 投资总额, miss 24616
            #'enttypegb',#企业（机构）类型] miss 0
            #### annual_report_info
            'WEBSITSIGN','FUNDAM','MEMNUM','FARNUM','ANNNEWMEMNUM','ANNREDMEMNUM',
            ]
...
if __name__ == "__main__":
    ...
    df.to_csv('submit_v2.csv', index=False) #submit
